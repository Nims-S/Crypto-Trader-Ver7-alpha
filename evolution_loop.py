...
def evolve_once(symbols, timeframes, children_per_parent=4, max_bars=0, allow_shorts=False, start=None, end=None, lookback_days=720, folds=3, train_ratio=0.6, val_ratio=0.2, test_ratio=0.2, use_cache=True, family="evo", seed=None):
    ...
    for symbol in symbols:
        for timeframe in timeframes:
            ...
            fb = (parent or {}).get("metrics") or {}
            children = [seed_strategy(symbol, timeframe, family=family)] if parent is None else mutate_parent(parent, symbol=symbol, timeframe=timeframe, n_children=children_per_parent, seed=seed, feedback=fb)
            ...
            for child in children:
                ...
                results.append({"strategy_id": child.strategy_id, "symbol": child.symbol, "timeframe": child.timeframe, "walk_forward": summary})
