...
def walk_forward_validate(symbol: str, timeframe: str, strategy_override: dict | None = None, folds: int = 2):
    ...
    return {
        ...
        "spread": spread,
        "diagnostics": _summarize_walk_forward(fold_reports),
        "folds": fold_reports,
    }
...
def _regime_template(regime: str) -> dict[str, Any]:
    if regime == "trend":
        return {
            ...
            "min_adx_shift": 3,
            "min_atr_rank_shift": 0.04,
            "min_bb_rank_shift": 0.03,
            "rsi_long_shift": 2,
            "rsi_short_shift": -1,
            "allow_shorts": False,
        }
    if regime == "breakout":
        return {
            ...
            "min_adx_shift": 1,
            "min_atr_rank_shift": 0.03,
            "min_bb_rank_shift": 0.04,
            "rsi_long_shift": 1,
            "rsi_short_shift": -1,
            "allow_shorts": False,
        }
    return {
        ...
        "use_breakout_filter": False,
        "min_adx_shift": -8,
        "min_atr_rank_shift": -0.10,
        "min_bb_rank_shift": -0.10,
        "rsi_long_shift": -8,
        "rsi_short_shift": 8,
        "allow_shorts": True,
    }
...
def _btc_expand_search_space(child_params: dict[str, Any], target_regime: str) -> dict[str, Any]:
    params = dict(child_params)
    params["allow_shorts"] = True
    if target_regime in {"mean_reversion", "breakout"}:
        params["min_adx"] = _clamp(_safe_float(params.get("min_adx", 16.0)) - 2.0, 8.0, 40.0)
        params["min_atr_rank"] = _clamp(_safe_float(params.get("min_atr_rank", 0.15)) - 0.03, 0.05, 0.95)
        params["min_bb_rank"] = _clamp(_safe_float(params.get("min_bb_rank", 0.15)) - 0.03, 0.05, 0.95)
    return params
...
def _rank(row: dict[str, Any]) -> float:
    ...
    return max(_safe_float(wf.get("score", m.get("wf_score", 0.0))), _safe_float(decision.get("score", 0.0))) + 0.25 * robustness


def _candidate_pool(all_strats: list[dict[str, Any]], symbol: str, timeframe: str, top_k: int) -> list[dict[str, Any]]:
    ...
    for row in pool:
        ...
        if len(diversified) >= max(3, top_k):
            break

    for row in pool:
        if row not in diversified:
            diversified.append(row)
        if len(diversified) >= max(5, 2 * max(1, top_k)):
            break

    return diversified[: max(1, top_k)]


def evolve_once(symbol: str, timeframe: str, top_k: int = 3):
    all_strats = list_strategies(active_only=False)
    parents = _candidate_pool(all_strats, symbol, timeframe, top_k)
    ...
    for p in parents:
        ...
        for target_regime, label in mutation_specs:
            ...
            if symbol.upper().startswith("BTC/"):
                child_params = _btc_expand_search_space(child_params, target_regime)
            ...
            upsert_strategy(
                ...
                tags=list(dict.fromkeys(list(p.get("tags") or []) + [symbol, timeframe, "evo", target_regime, label, "signal_expansion"])),
                source="evolution",
            )
